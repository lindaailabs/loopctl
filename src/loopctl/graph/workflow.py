"""LangGraph workflow: the M1 single-task local loop.

Pipeline: clarifying → planning → plan gate (interrupt) → executing → testing
→ (fixing ⇄ executing) → reporting → done. Failure classes from SPEC §5.3 are
detected and routed to `escalated`.

The workflow is engine-agnostic: the engine and notifier are injected so unit
tests can drive the loop with fakes and no network.
"""
...
class Workflow:
    ...
    async def resume(self, task_id: str) -> None:
        async with self._compiled() as graph:
            ...
            if snapshot.next:  # paused at an interrupt (plan gate)
    ...
    async def fixing(self, state: dict) -> dict:
        ...
        if task.fix_loops > self.project.limits.fix_loop_max:
    ...
    def _route_after_exec(self, state: dict) -> str:
        task = self._task(state)
        return "escalated" if task.state is TaskState.escalated else "testing"
    ...
    def _build(self):
        ...
        graph.add_edge("planning", "plan_gate")
        graph.add_conditional_edges(
            "plan_gate", self._route_after_gate, {"planning": "planning", "executing": "executing"}
        )
        graph.add_conditional_edges(
            "executing", self._route_after_exec, {"testing": "testing", "escalated": "escalated"}
        )
        graph.add_conditional_edges(
            "testing", self._route_after_test, {"reporting": "reporting", "fixing": "fixing"}
        )
        graph.add_conditional_edges(
            "fixing", self._route_after_fix, {"testing": "testing", "escalated": "escalated"}
        )
    ...
    def _ctx(self, task: Task, plan: str) -> EngineContext:
        limits: Limits = self.project.limits
...
def build_workflow(
    ...
    *,
    ...
    return Workflow(
        project=cfg, data_dir=data_dir, db_path=db_path, engine=engine, notifier=notifier
    )
